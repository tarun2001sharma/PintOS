...
// struct list thread_list;    //List of threads
...
//   bool curr_sleep = temp_a->sleep_ticks < temp_b->sleep_ticks;
...
static void real_time_sleep(int64_t num, int32_t denom)
{
	...
	ASSERT(intr_get_level() == INTR_ON);
	if (ticks > 0){
		/* wait for atleast 1 full timer tick*/
		timer_sleep(ticks);
	}
	else{
		/* Else use a busy-wait loop */
