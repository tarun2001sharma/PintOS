...
static void start_process(void *args_)
{
	...
	Curr__thread = thread_current();

	
	if (!success){
		palloc_free_page(file_name);	
		Curr__thread->return_status = RET_STATUS_ERROR;	
		sema_up(&Curr__thread->sema_process_load);
		thread_exit();
	}

	int Arguments = 0, argc = 0, Length;
	void* sp;
	...
	*((void**) (sp)) = 0;
	...
	Curr__thread->exec = filesys_open(file_name);

	
	file_deny_write(Curr__thread->exec);
