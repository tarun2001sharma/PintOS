...
static void page_fault(struct intr_frame *f)
{
	...
	if (page != NULL){
		if (write && !page->writable) system_call_exit(-1);
		bool success = VM_operation_page(OP_LOAD, page, page->physical_address, false);
		if (success) return;
		else system_call_exit(-1);
	}
	bool stack_status = ((uint32_t) fault_addr > 0) && (fault_addr >= (f->esp - 32)) && ((PHYS_BASE - pg_round_down(fault_addr)) <= (1 << 23));
	if (stack_status){
		struct page_struct *temp = VM_stack_grow(fault_page, false);
		if (temp != NULL) return;
		else system_call_exit(-1);
	}
	if (not_present) system_call_exit(-1);
	f->eax = 0xffffffff;
	f->eip = (void *) f->eax;
#endif
